...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.masks_data_root = Path(masks_data_root)
        if not self.masks_data_root.exists():
            raise ValueError("Masks images root doesn't exists.")
        self.mask_images_path = list(Path(masks_data_root).iterdir())
        masks_data_root
        ...
        self.image_transforms_ = transforms.Compose(
            [
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        ...
        masks_data_root=args.mask_data_dir,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        with TorchTracemalloc() if not args.no_tracemalloc else nullcontext() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                #noise_ = vae.encode(batch["noise_values"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                with accelerator.accumulate(unet):
                    ...
                    noise = vae.encode(batch["noise_values"].to(dtype=weight_dtype)).latent_dist.sample()
                    noise = noise * 0.18215
